...
class _DriverAuthWrapperState extends State<DriverAuthWrapper> {
  ...
  Future<void> _checkAuthState() async {
    ...
    try {
      ...
      debugPrint("Driver token refresh failed: $e, falling back to cached token");
      try {
        token = await user.getIdToken(false);
      } catch (_) {}
    }

    // Try verifying driver profile with retry logic (up to 3 attempts) for cold-start server resilience
    int attempts = 0;
    http.Response? res;
    while (attempts < 3) {
      try {
        ...
        debugPrint("Driver network/server check attempt ${attempts + 1} failed: $e");
      }
      attempts++;
    ...
    if (res != null && res.statusCode == 200) {
      ...
      // If driver is logged into Firebase but server is cold-starting or offline, DO NOT kick driver to Login screen!
      // Bypass directly into main app so driver remains logged in.
      Navigator.pushAndRemoveUntil(
        context,
        MaterialPageRoute(builder: (_) => DriverMainNavigation(driverData: {'name': 'Driver Partner', 'phone': user.phoneNumber ?? ''})),
        (route) => false,
      );
    } else {
